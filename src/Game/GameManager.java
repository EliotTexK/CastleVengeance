...
class GameManager
{
    ...
    private class LevelManager
    {
        ...
        public LevelManager(int width, int height, byte music)
        {
            ...
            tileSet.makePlatform(0,15,32,2);
            tileSet.makeWall(16,10,32,2);
        ...
        void update(Input input, double delta) {
            ...
            //ground collision
            int tileX = (int) Math.floor((player.getX() + (Constants.PLAYER_WIDTH/2)) / 32);
            int tileY = (int) Math.floor(player.getFeet() / 32);

            if (tileSet.getTiles()[tileX][tileY] > 0 && player.getVsp() >= 0)   {
                player.setY((tileY * 32) - Constants.PLAYER_HEIGHT);
                player.setVsp(0);
            ...
            //cieling collision
            tileX = (int) Math.floor((player.getX() +(Constants.PLAYER_WIDTH/2)) / 32);
            tileY = (int) Math.floor((player.getY() + 32) / 32);
            ...
            //wall collision
            tileX = (int) Math.floor((player.getX() + Constants.PLAYER_WIDTH) / 32);
            tileY = (int) Math.floor((player.getFeet()-1) / 32);

            if (tileSet.getTiles()[tileX][tileY] > 0 && tileSet.getTiles()[tileX][tileY] != 2 && player.getHsp() >= 0)   {
                player.setX((tileX * 32) - Constants.PLAYER_WIDTH);
            }

            tileX = (int) Math.floor(player.getX() / 32);
            tileY = (int) Math.floor((player.getFeet()-1) / 32);
            ...
            if (player.getX() + player.getHsp() > ((tileSet.getWidth()*2)-2)*32) {
                player.setX((tileSet.getWidth()-2)*32);
            }

            if (player.getFeet() + player.getVsp() > (tileSet.getLength()-2)*32) {
                player.setX(player.getSpawnX());
                player.setY(player.getSpawnY());
                camera.x = playerStartX;
                camera.y = playerStartY;
            ...
            scrolledY = camera.y - (320);
            scrolledX = camera.x - (240);
        }
        ...
        private class DrawScreen extends JPanel {
            ...
            public void paintComponent(Graphics g)
            {
                ...
                for (int i = 0; i < sprites.length; i++) {
                    if (!sprites[i].isFlip())   {
                        g2d.drawImage(spriteImages[sprites[i].getImage()], sprites[i].getX() - scrolledX, sprites[i].getY() - scrolledY, this);
                    }
                    if (sprites[i].isFlip())    {
                        g2d.drawImage(spriteImages[sprites[i].getImage()], sprites[i].getX() - scrolledX+32, sprites[i].getY() - scrolledY, -32, 48,this);
                    }
                }
